...
## D-023 — a write-capable token in Actions secrets, for OpenWiki — SUPERSEDED BY D-024 on 2026-09-14
...
## D-024 — OpenWiki generation is manual, not scheduled — RATIFIED 2026-09-14
...
`OPENWIKI_PR_TOKEN` and `ANTHROPIC_API_KEY` are not held as repository
secrets; if they were created, they are revoked.
...
**Why.** Three problems close at once: the repository no longer holds a
write-capable token; no scheduled job spends model budget unattended; and the
