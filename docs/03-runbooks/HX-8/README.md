...
refuses to run on any host other than `hx-8`.
...
2. Join `hx.local.arpa`; validate SSSD/domain user; install the pinned NVIDIA
   driver; reboot.
3. Install the Open WebUI application software.
...
5. Fill in `docs/02-server-records/HX-8.md`, then set `state` and
   `gate` for `HX-8` in `docs/00-control/hx-fleet.tsv` and run
