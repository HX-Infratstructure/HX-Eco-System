# HX-15 Current Runbook

**Host:** hx-15
**Expected IP:** `192.168.50.215`
**Role:** FastMCP shared/custom MCP development host

## Execution

Steps 1-2 are the shared base blocks. Run them from this directory; each one
refuses to run on any host other than `hx-15`.
...
2. Join `hx.local.arpa`; validate SSSD/domain user; install the pinned NVIDIA
   driver; reboot.
3. Install the FastMCP shared/custom MCP development host application software.
...
5. Fill in `docs/02-server-records/HX-15.md`, then set `state` and
   `gate` for `HX-15` in `docs/00-control/hx-fleet.tsv` and run
