...
> Scaffolded by `tools/hx-doc/hx-new-server hx-8`. Fill every section as the
...
|---|---|---|
| `hostname -f` | `hx-8.hx.local.arpa` | NOT ESTABLISHED |
...
| External key-only login | `tools/hx-doc/hx-fleet-access hx-8` returns `hx-8` and `KEY+SUDO-PASS` | NOT ESTABLISHED |
