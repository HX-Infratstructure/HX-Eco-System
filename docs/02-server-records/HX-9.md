...
> Scaffolded by `tools/hx-doc/hx-new-server hx-9`. Fill every section as the
...
|---|---|---|
| `hostname -f` | `hx-9.hx.local.arpa` | NOT ESTABLISHED |
...
| External key-only login | `tools/hx-doc/hx-fleet-access hx-9` returns `hx-9` and `KEY+SUDO-PASS` | NOT ESTABLISHED |
